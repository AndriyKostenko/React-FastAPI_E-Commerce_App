...
const FALLBACK_SIZE_PRICE_MULTIPLIERS: Record<keyof typeof SIZE_MEASUREMENTS, number> = {
  S: 1,
  M: 1,
  L: 1,
};
...
const HeroSection = () => {
  ...
  const finalPrice = useMemo(() => {
    ...
    const normalizedSize = (size in sizePriceMultipliers ? size : "M") as keyof typeof sizePriceMultipliers;
  ...
  const handleAddToCart = () => {
    ...
    const unitPrice = Math.round((finalPrice / Math.max(quantity, 1)) * 100) / 100;
