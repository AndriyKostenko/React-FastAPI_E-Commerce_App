...
async def add_product_images(
    ...
    new_product_images = await image_service.create_product_images(
        product_id=product_id, images=image_metadata
    )
...
async def replace_product_images(
    ...
    if len(image_paths) != len(processed_colors) or len(image_paths) != len(
        processed_color_codes
    ):
        raise HTTPException(
    ...
    if any(len(color) < 3 for color in processed_colors) or any(
        len(code) < 3 for code in processed_color_codes
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # Create image data
    image_data = [
        ImageType(image_url=path, image_color=color, image_color_code=code)
        for color, code, path in zip(
            processed_colors, processed_color_codes, image_paths
        )
    ]
