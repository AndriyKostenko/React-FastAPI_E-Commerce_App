...
class RedisManager:
    ...
    def _generate_cache_key(self,
        ...
        self.logger.debug(f"Generating cache key: {cache_key}, Original URL: {request.url}, Raw path: {path},Query params: {query_params}")
    ...
    async def get_cached_response(self, request: Request) -> Optional[JSONResponse]:
        ...
        cache_key = self._generate_cache_key(request=request)
        if not cache_key:
            self.logger.error("Cannot generate cache key for getting cached response, skipping cache retrieval.")
            return None
