...
class RateLimiter:
    def __init__(self, times: int, seconds: int):
        ...
        self.redis = aioredis.from_url(
            f"redis://{settings.REDIS_ENDPOINT}:{settings.REDIS_PORT}",
            encoding="utf-8",
    ...
    def _generate_key(self, request: Request) -> str:
        ...
        """
        logger.debug(f"Generating rate limit key for request: {request.client.host} : {request.url.path}")
    ...
    async def __call__(self, request: Request):
        ...
        if not await self._check_rate_limit(key):
            ...
            raise RateLimitExceededError(detail={'message': 'Too many requests.',
