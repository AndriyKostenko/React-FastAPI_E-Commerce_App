...
@user_service_redis_manager.ratelimiter(
    times=5, seconds=3600
)  # Limit to 5 registrations per hour
async def create_user(
    request: Request, data: UserSignUp, user_service: user_crud_dependency
):
    ...
    verification_token, _ = auth_manager.create_access_token(
...
@user_service_redis_manager.ratelimiter(
    times=5, seconds=3600
)  # Limit to 5 verifications per hour
async def verify_email(
    request: Request, token: str, user_service: user_crud_dependency
):
...
async def forgot_password(
    request: Request, email: EmailStr, user_service: user_crud_dependency
):
...
async def reset_password(
    ...
    return JSONResponse(
        content=PasswordUpdateResponse(
            detail="Password reset successfully", email=token_data.email
        ),
        status_code=status.HTTP_200_OK,
    )
...
async def login(
    ...
    user_service: user_crud_dependency,
):  # request is reqired for rate limiting decorator
    ...
    access_token, expiry_timestamp = auth_manager.create_access_token(
        email=user.email,
        user_id=user.id,
        role=user.role,
