...
from shared.shared_instances import logger, settings, user_service_database_session_manager
from exceptions.outbox_event_exceptions import OutboxEventNotFoundError
from service_layer.outbox_event_service import OutboxEventService
from events_publisher.user_events_publisher import user_events_publisher, UserEventPublisher
...
class OutboxPollerService:
    ...
    def __init__(self) -> None:
        self.session_manager: DatabaseSessionManager = user_service_database_session_manager
        self.user_event_publisher: UserEventPublisher = user_events_publisher
    ...
    async def event_type_to_publisher(self, event_data: dict[str, Any]):
        ...
        match event_type:
            case UserEvents.USER_REGISTERED:
                await self.user_event_publisher.publish_user_registered(event_data)
            case UserEvents.USER_EMAIL_VERIFIED:
                await self.user_event_publisher.publish_email_verified(event_data)
            case UserEvents.USER_PASSWORD_RESET_REQUEST:
                await self.user_event_publisher.publish_password_reset_request(event_data)
            case UserEvents.USER_PASSWORD_RESET_SUCCESS:
                await self.user_event_publisher.publish_password_reset_success(event_data)
            case UserEvents.USER_LOGGED_IN:
                await self.user_event_publisher.publish_user_logged_in(event_data)
            case UserEvents.USER_DELETED:
                await self.user_event_publisher.publish_user_deleted(event_data)
            case _:
